def phoenix_posdc(ptype,num,freq,delta,rho,cond,r,k,maxeval):

    from readttp import readttp, dynp, findk, np, pd, re
    from posdc_script import oneplusone, individual_multi, posdc
    import copy

    ptype = int(ptype)
    freq = int(freq)
    delta = int(delta)
    rho = float(rho)
    cond = int(cond)
    r = int(r)
    k = int(k)  # SampleTime
    eta=r
    num = int(num)
    maxeval = int(maxeval)

    if ptype == 1:
        ProblemName = 'eil101_n' + str(num) + '_uncorr_01.ttp'
        Ptype = 'uncorr'
    elif ptype == 2:
        ProblemName = 'eil101_n' + str(num) + '_bounded-strongly-corr_01.ttp'
        Ptype = 'bsc'
    # elif ptype == 3:
    #     ProblemName = 'eil101_n' + str(num) + '_uncorr-similar-weights_01.ttp'
    #     Ptype = 'usw'

    FinalErrList = []
    FinalErrStr = 'MyTxt/posdc_' + str(Ptype) + '_' + str(num) + '_' + str(cond) + '_' + str(r) + '_' + str(freq) + '_' + str(
        delta) + '_' + str(rho) + '_' + str(k + 1) + '.txt'

    print(FinalErrStr)

    NumberItems = int(re.findall(r'\d+', ProblemName)[1])

    (df, InitialCapacity) = readttp(ProblemName)

    w = np.array(df.loc[:]['weight']).astype('int64')  # weight in the instance
    p = np.array(df.loc[:]['profit']).astype('int64')  # profit in the instance

    million = maxeval
    maxp = max(p)  # maximum profit in the instances
    maxweight = sum(w)  # total nominal weight possible
    # # Dynamic Table
    m = np.zeros([NumberItems + 1, sum(w) + 1])
    DynamicTable = dynp(w, p, m, m.shape[1] - 1)
    # # shift in inital capacity
    woriginal = w - 100
    NewCapacity = findk(woriginal, InitialCapacity)

    # NewCapacity=InitialCapacity
    binary_string_length = NumberItems

    ### Magnitude of changes:
    DistStr = 'Dist/uniform_dist_' + str(r) + '.csv'
    CapacityTable = pd.read_csv(DistStr, header=None)
    Freq = int(million) / freq
    Iter = 1
    ## 1e4 initial stage
    solution = individual_multi(NumberItems)
    solution.evaluate(NewCapacity, w, rho, delta, cond, p)
    # check if the solution is in our desired frame
    if not (solution.sbound <= NewCapacity + eta and solution.sbound >= NewCapacity - eta):
        # repair it with oneplusone:
        repair_alg = oneplusone(solution, NewCapacity, w, p, maxp, delta, rho, cond, r,
                                int(DynamicTable[-1, NewCapacity]))
        count = 0
        while (count <= 1e4 and not (
                repair_alg.individual.sbound <= NewCapacity + eta and repair_alg.individual.sbound >= NewCapacity - eta)):
            repair_alg.mutate(NumberItems)
            count += 1

    initial_alg = posdc(repair_alg.individual, [], [], NewCapacity, w, p, maxp, delta, rho, cond, r, eta,
                        int(DynamicTable[-1, NewCapacity]))

    for Iter in range(int(1e4)):
        initial_alg.mutate()
        # initial_alg.calc_err()

    # solution=copy.deepcopy(initial_alg.individual)
    # print(initial_alg.finalerr[-1],Iter)

    solutions = initial_alg.sminus + initial_alg.splus
    # print('Initial Phase is finished with computing time of: ',str(algorithm.total_computing_time))
    print('Initial Phase is finished with computing time of: ')

    for f in range(int(Freq)):
        NewCapacity += CapacityTable.iloc[f, k]
        if NewCapacity >= maxweight:
            NewCapacity = maxweight
        elif NewCapacity <= 0:
            NewCapacity = 0

        ### Update Splus & Sminus:

        sminus = []
        splus = []

        # evaluate available solutions again:
        for item in solutions:
            item.evaluate(NewCapacity, w, rho, delta, cond, p)

        for i in solutions:
            if i.sbound <= NewCapacity + eta and i.sbound >= NewCapacity - eta:
                if i.sbound <= NewCapacity:
                    sminus.append(i)
                else:
                    splus.append(i)

        # find best solution:
        if len(sminus) > 0:  # the solution with highest profit should be chosen
            best_solution = sminus[np.argmax([np.abs(i.profit) for i in sminus])]  # maximize the profit
        elif len(splus) >0 and len(sminus)==0:
            best_solution = splus[np.argmin([i.sbound for i in splus])]  # minimize the profit
        else:
            # repair it with oneplusone:
            repair_alg = oneplusone(algorithm.solution, NewCapacity, w, p, maxp, delta, rho, cond, r,
                                    int(DynamicTable[-1, NewCapacity]))
            count = 0
            while (count <= 1e4 and not (
                    repair_alg.individual.sbound <= NewCapacity + eta and repair_alg.individual.sbound >= NewCapacity - eta)):
                repair_alg.mutate(NumberItems)
                count += 1
            best_solution = repair_alg.individual

        algorithm = posdc(best_solution, sminus, splus, NewCapacity, w, p, maxp, delta, rho, cond, r, eta,
                          int(DynamicTable[-1, NewCapacity]))
        max_iterations = int(freq)
        for j in range(max_iterations):
            algorithm.mutate()
            algorithm.calc_err()
        if np.mod(f, 100) == 0:
            print('Capacity: {}, Freq: {}, FinalErr: {}'.format(NewCapacity, f, algorithm.finalerr[-1]))
        solutions = copy.deepcopy(algorithm.sminus + algorithm.splus)
        FinalErrList.extend(algorithm.finalerr)

        # if np.mod(f,100)==0:

    # POSDC Output:
    FinalErr = np.sum(FinalErrList) / million

    with open(FinalErrStr, 'w') as of:
        of.write(str(FinalErr))
        of.write('\n')

    print('Output is done')
